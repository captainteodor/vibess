...
export const LoginWithGoogleButton = () => {
  ...
  const handleSignInSuccess = async (user: any) => {
    ...
    // Initialize analytics after successful sign-in (only on web)
    if (Platform.OS === 'web') {
      try {
        const { initializeAnalytics } = await import('../lib/analytics/analytics');
        await initializeAnalytics();
        console.log('Analytics initialized after sign-in');
      } catch (error) {
  ...
  const onGoogleButtonPress = async (): Promise<GoogleSignInResult> => {
    try {
      if (Platform.OS === 'web') {
        ...
        const { GoogleAuthProvider, signInWithPopup } = googleSignIn;
        
        if (!GoogleAuthProvider || !signInWithPopup) {
          throw new Error('Google Sign-In not properly loaded');
        }
        ...
        const googleSignIn = await getGoogleSignIn();
        const { GoogleAuthProvider, signInWithRedirect } = googleSignIn;
        
        if (!GoogleAuthProvider || !signInWithRedirect) {
          throw new Error('Google Sign-In not properly loaded');
        }
